...
function ProductInfo({ activeProduct, activeVariantImage, onVariantSelect }) {
  ...
  return (
    <div className="text-center w-full max-w-5xl mx-auto backdrop-blur-sm rounded-2xl pt-10 pb-6 px-6">
      {/* PRODUCT TITLE - Letter-by-letter Staggered Swipe Up */}
      <div className="relative w-full flex items-center justify-center mb-1" style={{ minHeight: '6rem', paddingBottom: '0.2rem' }}>
        <AnimatePresence mode="wait">
          <motion.h2
            key={activeProduct.id}
            layoutId={`product-title-${activeProduct.id}`}
      ...
      {uniqueVariants.length > 0 && (
        <div className="flex items-center justify-center mb-4 gap-3 flex-shrink-0 relative z-10">
      ...
      <div className="relative w-full flex items-center justify-center" style={{ minHeight: '3.5rem' }}>
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className="text-gray-500 secondary text-[10px] sm:text-[11px] md:text-[12px] lg:text-[13px] leading-relaxed text-center w-full m-0 max-w-md mx-auto px-4"
          >
      ...
      <div className="relative w-full flex items-center justify-center mt-2" style={{ minHeight: '4rem' }}>
        <AnimatePresence mode="wait">
          <motion.div
            key={activeProduct.id}
...
function MainCarousel({ products, activeIndex, setActiveIndex, activeVariantImage }) {
  ...
  return (
    ...
    >
        ...
        return (
          ...
          >
            <div className="relative w-full h-full flex justify-center items-center pointer-events-auto">
              {/* Bottom Sinking Shadow - Fixed size */}
              <div
                className="absolute bottom-5 left-1/2 -translate-x-1/2 w-[200px] h-[25px] bg-black/20 blur-xl rounded-[100%] z-0"
              ...
              {/* Product Image - Stable rendering */}
              <img
                src={displayImage}
                alt={product.name}
                className={`absolute top-0 left-0 w-full h-full object-contain drop-shadow-xl z-20 transition-all duration-500 ${isCurrent ? "opacity-100 scale-100" : "opacity-60 grayscale-[0.5] scale-90"}`}
              />
            </div>
          </motion.div>
        );
      ...
      >
        <ArrowLeft className="h-5 w-5 sm:h-6 sm:w-6 text-black transition-colors" strokeWidth={3} />
      </button>
      <button
        onClick={handleNext}
        ...
        style={{ boxShadow: '8px 8px 16px #d1d1d1, -8px -8px 16px #ffffff' }}
      >
        <ArrowRight className="h-5 w-5 sm:h-6 sm:w-6 text-black transition-colors" strokeWidth={3} />
      </button>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <section className="cursor-default w-full min-h-screen pt-32 pb-24 relative overflow-hidden flex flex-col items-center bg-[#ebebeb]">
      <TabSwitcher
        activeTab={activeTab}
